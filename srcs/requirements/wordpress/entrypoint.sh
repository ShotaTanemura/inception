...
sed -i "s/\REDIS_USER/${REDIS_USER}/g" "$wp_config_file"
sed -i "s/\REDIS_PASSWORD/${REDIS_PASSWORD}/g" "$wp_config_file"
...
wp plugin install redis-cache --allow-root --path=/var/www/html/wordpress --activates;
